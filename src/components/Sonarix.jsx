...
import { Activity, AlertTriangle, TrendingUp, Download, Settings, LogOut, Calendar, Filter, RefreshCw, Eye, FileText, Zap, Clock, Shield, Brain, X } from 'lucide-react';
const Sonarix = () => {
    ...
    const TabButton = ({ tabId, children }) => (<button onClick={() => setActiveTab(tabId)} className={`px-3 py-2 text-sm font-medium transition-colors ${activeTab === tabId
            ? 'text-purple-600 border-b-2 border-purple-600'
            : 'text-gray-500 hover:text-gray-700'}`}>
            {children}
        </button>);
    const MetricCard = ({ title, value, icon: Icon, trend }) => (<div className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center justify-between">
                <div>
                    <p className="text-sm text-gray-500">{title}</p>
                    <p className="text-2xl font-bold text-gray-900">{value}</p>
                    {trend !== undefined && (<p className={`text-xs mt-1 ${trend > 0 ? 'text-green-600' : 'text-red-600'}`}>
    ...
    const AnomalyCard = ({ anomaly }) => {
        ...
        return (<div className={`p-4 rounded-lg border-l-4 ${bgColor}`}>
    ...
    return (<div className="min-h-screen bg-gray-50 text-gray-800">
            {error && (<div className="fixed top-4 right-4 bg-red-500 text-white px-4 py-2 rounded-lg shadow-lg z-50 animate-pulse">
                    {error}
                </div>)}
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                {activeTab === 'dashboard' && (<div className="space-y-6">
                        <div className="bg-white rounded-lg shadow p-6">
                            <div className="flex justify-between items-center mb-4">
                                ...
                                <button onClick={() => setShowConnectModal(true)} className="bg-purple-600 text-white px-4 py-2 rounded-lg hover:bg-purple-700 transition-colors">
                                    Подключить аккаунт
                                </button>
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                {['Яндекс.Директ', 'Яндекс.Метрика'].map((platform) => (<div key={platform} className={`p-4 rounded-lg border-2 transition-colors ${connectedAccounts.includes(platform)
                    ? 'border-green-500 bg-green-50'
                    : 'border-gray-200 bg-gray-50'}`}>
                                        <div className="flex items-center justify-between">
                                            <span className="font-medium">{platform}</span>
                                            {connectedAccounts.includes(platform) ? (<div className="flex items-center text-green-600">
                                                    <Shield className="h-4 w-4 mr-1"/>
                                                    <span className="text-sm">Подключен</span>
                                                </div>) : (<span className="text-sm text-gray-500">Не подключен</span>)}
                                        </div>
                                    </div>))}
                            </div>
                        </div>
                            ...
                            <div className="bg-white rounded-lg shadow p-6">
                                ...
                                <ResponsiveContainer width="100%" height={300}>
                                    <LineChart data={performanceData}>
                                        ...
                                        <Tooltip />
                                        <Legend />
                                        <Line type="monotone" dataKey="clicks" stroke="#755D9A" name="Клики" strokeWidth={2} dot={{ r: 4 }} activeDot={{ r: 6 }}/>
                                        <Line type="monotone" dataKey="conversions" stroke="#C0C0C0" name="Конверсии" strokeWidth={2} dot={{ r: 4 }} activeDot={{ r: 6 }}/>
                                    </LineChart>
                                </ResponsiveContainer>
                            </div>
                            <div className="bg-white rounded-lg shadow p-6">
                        ...
                        <div className="bg-white rounded-lg shadow p-6">
                            ...
                            <div className="mb-6">
                                ...
                                <div className="flex items-center space-x-4">
                                    <div className="flex items-center">
                                        ...
                                        <input type="date" value={dateRange.start} onChange={(e) => handleDateChange('start', e.target.value)} className="border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-500"/>
                                    </div>
                                    <span className="text-gray-500">—</span>
                                    <input type="date" value={dateRange.end} onChange={(e) => handleDateChange('end', e.target.value)} className="border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-500"/>
                                </div>
                            </div>
                            ...
                            <div className="mb-6">
                                <div className="flex justify-between items-center mb-3">
                                    ...
                                    <button onClick={() => setShowFilters(!showFilters)} className="flex items-center text-sm text-purple-600 hover:text-purple-700">
                                        <Filter className="h-4 w-4 mr-1"/>
                                        Фильтры
                                    </button>
                                </div>

                                {showFilters && (<div className="bg-gray-50 rounded-lg p-4 mb-4">
                                ...
                                <div className="space-y-2">
                                    {campaignData.map((campaign) => (<div key={campaign.id} onClick={() => toggleCampaignSelection(campaign.id)} className={`p-4 rounded-lg border-2 cursor-pointer transition-all ${selectedCampaigns.includes(campaign.id)
                    ? 'border-purple-600 bg-purple-50'
                    : 'border-gray-200 hover:border-gray-300'}`}>
                                            <div className="flex items-center justify-between">
                                                <div className="flex items-center">
                                                    <input type="checkbox" checked={selectedCampaigns.includes(campaign.id)} onChange={() => toggleCampaignSelection(campaign.id)} onClick={(e) => e.stopPropagation()} className="mr-3 h-4 w-4 text-purple-600 rounded focus:ring-purple-500"/>
                                                    <div>
                            ...
                            <div className="flex justify-between items-center">
                                ...
                                <button onClick={handleGenerateReport} disabled={selectedCampaigns.length === 0 || isGeneratingReport} className={`flex items-center px-6 py-3 rounded-lg font-medium transition-all ${selectedCampaigns.length === 0 || isGeneratingReport
                ...
                : 'bg-purple-600 text-white hover:bg-purple-700'}`}>
                            ...
                            {isGeneratingReport && (<div className="mt-4">
                                    <div className="w-full bg-gray-200 rounded-full h-2">
                                        <div className="bg-purple-600 h-2 rounded-full transition-all duration-500 ease-out" style={{ width: `${reportProgress}%` }}/>
                                    </div>
                                </div>)}
                        </div>
                ...
                {activeTab === 'reports' && (<div className="space-y-6">
                        <div className="bg-white rounded-lg shadow p-6">
                            ...
                            <div className="space-y-3">
                                {reports.map((report) => (<div key={report.id} className="flex items-center justify-between p-4 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
                                        <div className="flex items-center">
            ...
            {showConnectModal && (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                    <div className="bg-white rounded-lg shadow-xl p-6 w-96 max-w-[90vw]">
                        ...
                        <div className="space-y-3">
                            {['Яндекс.Директ', 'Яндекс.Метрика'].map((platform) => (<button key={platform} onClick={() => handleConnectAccount(platform)} disabled={connectedAccounts.includes(platform) || connectingAccount === platform} className={`w-full p-3 rounded-lg border-2 transition-all ${connectedAccounts.includes(platform)
                    ? 'border-gray-300 bg-gray-100 text-gray-400 cursor-not-allowed'
                    : connectingAccount === platform
                        ? 'border-purple-400 bg-purple-50 text-purple-600'
                        : 'border-purple-600 text-purple-600 hover:bg-purple-50'}`}>
                                    {connectedAccounts.includes(platform) ? ('Уже подключен') : connectingAccount === platform ? (<span className="flex items-center justify-center">
                                            <RefreshCw className="h-4 w-4 mr-2 animate-spin"/>
                                            Подключение...
                                        </span>) : (`Подключить ${platform}`)}
                                </button>))}
                        </div>
                    </div>
                </div>)}
...
//# sourceMappingURL=Sonarix.jsx.map